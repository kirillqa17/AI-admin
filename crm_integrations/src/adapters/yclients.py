"""
YCLIENTS CRM Adapter

Документация API: https://yclientsru.docs.apiary.io/
API v2.0 Base URL: https://api.yclients.com/api/v1/

Особенности:
- Bearer token авторизация (partner_token + user_token)
- Rate limit: 5 запросов/секунду
- Max 200 результатов на запрос
"""
...
class YClientsAdapter(BaseCRMAdapter):
    ...
    YCLIENTS - лидер в России для салонов красоты, фитнеса, медицинских центров.
    ...
    def __init__(
        ...
        **kwargs
        ...
        Args:
            api_key: Partner Token (Bearer token)
            base_url: Базовый URL API (по умолчанию используется стандартный)
            company_id: ID компании в YCLIENTS
    ...
    async def _request(
        ...
        try:
            ...
            # YCLIENTS возвращает {"success": true, "data": ..., "meta": ...}
            if isinstance(data, dict) and "success" in data:
                if not data.get("success"):
                    error_msg = data.get("meta", {}).get("message", "Unknown error")
                    logger.error("yclients_api_error", error=error_msg)
                    raise Exception(f"YCLIENTS API error: {error_msg}")
            ...
            logger.error("yclients_request_error", error=str(e), endpoint=endpoint)
            raise
    ...
    async def authenticate(self, login: str, password: str) -> str:
        ...
        logger.info("yclients_authenticated", user_id=data.get("id"))
    ...
    async def get_client_by_phone(self, phone: str) -> Optional[CRMClient]:
        ...
        try:
            ...
            logger.warning("yclients_client_search_failed", phone=phone[-4:], error=str(e))
            return None
    ...
    async def create_client(self, client: CRMClient) -> CRMClient:
        ...
        logger.info("yclients_client_created", client_id=data.get("id"))
    ...
    async def update_client(self, client: CRMClient) -> CRMClient:
        ...
        logger.info("yclients_client_updated", client_id=client.id)
        return self._parse_client(data)

    def _parse_client(self, data: Dict) -> CRMClient:
        """Парсинг данных клиента из ответа API"""
        return CRMClient(
            id=str(data.get("id")),
            ...
            custom_fields={"yclients_data": data}
        )
    ...
    async def get_services(
        ...
        for item in data if isinstance(data, list) else [data]:
            # YCLIENTS возвращает услуги сгруппированные по категориям
            if "services" in item:
                # Это категория с услугами
        ...
        logger.info("yclients_services_fetched", count=len(services))
        return services
    ...
    async def get_service_by_id(self, service_id: str) -> Optional[CRMService]:
        ...
        try:
            ...
            logger.warning("yclients_service_not_found", service_id=service_id)
            return None
    ...
    async def get_employees(
        ...
        logger.info("yclients_employees_fetched", count=len(employees))
        return employees
    ...
    async def get_employee_by_id(self, employee_id: str) -> Optional[CRMEmployee]:
        ...
        try:
            ...
            logger.warning("yclients_employee_not_found", employee_id=employee_id)
            return None
    ...
    async def get_available_slots(
        ...
        for slot_date in available_dates:
            # Если сотрудник не указан, получаем всех доступных
            if employee_id:
        ...
        logger.info("yclients_slots_fetched", count=len(slots))
        return slots
    ...
    async def create_appointment(self, appointment: CRMAppointment) -> CRMAppointment:
        ...
        payload = {
            "phone": "",  # Нужно получить из клиента
            "fullname": "",
            "email": "",
            "appointments": [{
                "id": 1,
        ...
        # Если есть client_id, получаем данные клиента
        if appointment.client_id:
            try:
                ...
                payload["phone"] = client_data.get("phone", "")
                payload["fullname"] = client_data.get("name", "")
                payload["email"] = client_data.get("email", "")
            except Exception:
                pass
        ...
        logger.info("yclients_appointment_created", record_id=record_id)
    ...
    async def get_appointment_by_id(self, appointment_id: str) -> Optional[CRMAppointment]:
        ...
        try:
            ...
            logger.warning("yclients_appointment_not_found", appointment_id=appointment_id)
            return None
    ...
    async def cancel_appointment(self, appointment_id: str) -> bool:
        ...
        try:
            ...
            logger.info("yclients_appointment_cancelled", appointment_id=appointment_id)
            return True
        except Exception as e:
            logger.error("yclients_cancel_failed", appointment_id=appointment_id, error=str(e))
            return False
    ...
    def _parse_appointment(self, data: Dict) -> CRMAppointment:
        ...
        # Получаем ID первой услуги
        services = data.get("services", [])
        service_id = str(services[0].get("id")) if services else ""

        # Маппинг статусов YCLIENTS -> наши статусы
        status_map = {
            0: "pending",
            1: "confirmed",
            2: "completed",
